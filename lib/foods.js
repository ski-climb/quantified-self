...
$('document').ready(function() {
  ...
  // Calculate total and remaining calories for breakfast meal table
  $('.breakfast tr.food-row').each(function(){
    var food = $(this);
    var totalCalories = 0;

    $('.breakfast tr:not(:first,.total-calories-row,.remaining-calories-row)').each(function(){
      totalCalories += parseInt($(this).children().eq(food.index()).html());
    });

    var remainingCalories = 2000 - totalCalories;

    $('.breakfast tr.total-calories-row').children('td.total-calories-calc').html(totalCalories);
    $('.breakfast tr.remaining-calories-row').children('td.remaining-calories-calc').html(remainingCalories);
  });
