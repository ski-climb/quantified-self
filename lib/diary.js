...
$('document').ready(function() {
  ...
  function populateAllMealTablesForDay() {
    ...
    day["breakfast"].forEach(function(id) {
      var food = findFoodByID(id);
      addFoodToMealTable(food.name, food.calories, food.id, "breakfast")
    })
  }
  ...
  $('#add-breakfast').on("click", function(){
    ...
    if (mealDay) {
      updateMealDay(mealDay, "breakfast", mealIDs);
    } else {
      mealDay = new Day(date, "breakfast", mealIDs);
